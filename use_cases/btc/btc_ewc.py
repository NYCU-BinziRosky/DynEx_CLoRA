...
filtered_prev_state_dict = {k: v for k, v in checkpoint['model_state_dict'].items()
                            if k in current_model.state_dict() and v.size() == current_model.state_dict()[k].size()}
current_model.load_state_dict(filtered_prev_state_dict, strict=False)

ewc_object = EWC(fisher=checkpoint['ewc_fisher'], params=checkpoint['ewc_params'])
criterion = nn.CrossEntropyLoss()
