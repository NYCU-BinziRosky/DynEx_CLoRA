...
def train_with_mse_distillation(student_model, output_size, criterion, optimizer,
    ...
    """
    General training loop for MSE-based knowledge distillation.

    Args:
        student_model (nn.Module): Model to be trained.
        output_size (int): Number of output classes.
        criterion (Loss): Main classification loss (e.g., CrossEntropyLoss).
        optimizer (Optimizer): Optimizer for student model.
        X_train, y_train, X_val, y_val (np.ndarray): Training and validation data.
        stable_classes (list[int], optional): Subset of classes to distill from teacher.
        teacher_model (nn.Module, optional): Frozen model for distillation.
        alpha (float): Balance between distillation loss and classification loss.
        ...
        stop_signal_file (str): Path to a signal file for early stopping.
        device (torch.device): Device to use (default: auto-selected).
    """
